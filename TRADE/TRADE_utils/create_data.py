...
fin = open('TRADE_utils/mapping.pair','r')
replacements = []
...
def createData():
    ...
    fin1 = open(args.data_dir + '/data.json', 'r')
    data = json.load(fin1)

    fin2 = open(args.data_dir + '/system_acts.json', 'r')
...
def divideData(data):
    ...
    testListFile = []
    fin = open(args.data_dir + '/testListFile.txt', 'r')
    for line in fin:
        testListFile.append(line.strip())
    fin.close()

    valListFile = []
    fin = open(args.data_dir + '/valListFile.txt', 'r')
    for line in fin:
        valListFile.append(line.strip())
    fin.close()

    trainListFile = open(args.data_dir + '/trainListFile.txt', 'w')
    ...
    # save all dialogues
    with open(args.data_dir+'/trade_dev_dials.json', 'w') as f:
        json.dump(val_dials, f, indent=4)

    with open(args.data_dir+'/trade_test_dials.json', 'w') as f:
        json.dump(test_dials, f, indent=4)

    with open(args.data_dir+'/trade_train_dials.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, required = True,
                help="data dir to creat DST dataset")
