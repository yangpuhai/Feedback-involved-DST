...
SLOT=['restaurant-price range','restaurant-food','restaurant-area']
...
def prepare_dataset(data_path, feedback_data_path, train_feedback, tokenizer, slot_meta, n_history, max_seq_length,
                    op_code='4', data_type = ''):
    ...
    for dial_dict in dials:
        ...
        for ti, turn in enumerate(dial_dict["dialogue"]):
            ...
            if data_type == 'train':
                turn_uttr = (turn['system_transcript'] + ' ; ' + turn['transcript']).strip().lower() 
            else:
                turn_uttr = (turn['system_transcript'] + ' ; ' + turn['asr'][0][0]).strip().lower()
            ...
            if (turn_id + 1) == len_turns:
                task_final = 1
                session_final = 1
            instance = TrainingInstance(dialogue_id, 'restaurant',
                                        turn_id, turn_uttr, ' '.join(dialog_history[-n_history:]),
                                        deepcopy(last_dialog_state), turn_dialog_state, op_labels,
                                        generate_y, gold_state, max_seq_length, slot_meta,
                                        task_final, session_final, op_code=op_code)
            ...
            if data_type == 'train':
                ...
                if fid in feedback_set:
                    new_turn_uttr = turn_uttr + ' ' + feedback_set[fid]["feedback"]
                    new_turn_dialog_state = process_state(feedback_set[fid]["belief_state"], slot_meta)
                    ...
                    op_labels, generate_y, gold_state = make_turn_label(slot_meta, last_dialog_state,
                                                                new_turn_dialog_state,
                                                                tokenizer, op_code)
                    instance = TrainingInstance(dialogue_id, 'restaurant',
                                        turn_id, new_turn_uttr, ' '.join(dialog_history[-n_history:]),
                                        deepcopy(last_dialog_state), new_turn_dialog_state, op_labels,
                                        generate_y, gold_state, max_seq_length, slot_meta,
                                        task_final, session_final, op_code=op_code)
                    instance.make_instance(tokenizer)
                    data.append(instance)
...
class TrainingInstance:
    ...
    def make_instance(self, tokenizer, max_seq_length=None,
        ...
        len1 = len(diag_1)+len(diag_2)+len(state)
